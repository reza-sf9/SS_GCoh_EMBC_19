function [Zi, h] = ft_plot_topo_rs_2(config1)
% for reviewing help, check original fuction in FIELDTRIP TOOLBOX
%iNPUTS
% chanX          = (a vector) X position of channels
% chanY          = (a vector) Y position of channels
% dat            = (a vector) data that we want to plot topology of it
% win_num        = number of window that we want to plot topology of EIGENVECTORS OF IT
% colorbar_limit = specifies the min and max value of colorbar
% str_colormap   = specifies colormap

str_description = config1.str_description;
colorbar_limit = config1.colorbar_limit;
str_colormap = config1.color_map;
chanX = config1.ch_x;
chanY = config1.ch_y;
dat = config1.data;

%%%% REZA 
% Loading from saved data of position of electrods of 64 channels(from ft_topoplotER)
% We can get 

% load('layout_easycapM11_rs.mat')
% pos = layout_rs.pos;
% chanX = pos(:,1);
% chanX = chanX(1:64);
% chanY = pos(:,2);
...
% load data for varargin (I saved this data before from original fieldtrip implementation)
load('D:\Research\ali_2\real_4_9_new\new_topo\topo_plot\intial_ft_plot_topo.mat');
varargin = intial_ft_plot_topo.varargin;
...
%%%%%% REZA CODE
colorbar
caxis([colorbar_limit(1) colorbar_limit(2)])

colormap(str_colormap)

title({str_description});

% switch str_description
%     case 'Corresponding Largest Eigenvectors'
%         str_order = 'largest';
%     case 'Corresponding Second-Largest Eigenvectors'
%         str_order = 'secondLargest';
%         
% end
% 
% str_name = sprintf('%s_%s_topo_eigVec_ch%d_WinNum_%d.png' , str_order , str_colormap , length(chanX) , win_num);
% saveas(gcf , str_name);
I = getimage(gcf);
% close;
clc;
...
set(gcf, 'PaperPosition', [0 0 4.75 4]);
str_save = sprintf('topo_fr%d_min%d', config1.fr ,config1.min);
print(str_save ,'-dpng','-r600')
