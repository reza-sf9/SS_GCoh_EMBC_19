unction [Zi, h] = vs_ft_plot_topo_rs_2_6(config_topoPlot)

chanX = config_topoPlot.ch_x;
chanY = config_topoPlot.ch_y;
data = config_topoPlot.data;
colorbar_limit = config_topoPlot.colorbar_limit;
...
if ~isempty(maskimage) && strcmp(interplim, 'mask_individual')
  ...
  for i=1:max(maskimage(:))
    ...
    [Xi, Yi, tmpZi]  = griddata(chanX(chansel)', chanY(chansel), data(chansel), xi', yi, interpmethod);
...
% title({'topology plot of EIGENVECTORS for 64 channels',str_description});
